...
**Built with Claude Code** as a development assistant for architecture designs, the quality
...
`WHERE`, `IF-THEN-ELSE`, `RETAIN` (rule engine flags the block; dashboard shows LLM-produced window function equivalent), date functions (`TODAY()`, `MDY()`),
